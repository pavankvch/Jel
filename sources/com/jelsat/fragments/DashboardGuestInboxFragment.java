...
public class DashboardGuestInboxFragment extends BaseFragment implements OnTabSelectedListener, IInboxView {
    @BindView(2131362641)
    TabLayout inboxTabs;
    ...
    private void initSwipeToRefresh() {
        this.swipeContainer.setOnRefreshListener(new DashboardGuestInboxFragment$1(this));
        this.swipeContainer.setColorSchemeResources(17170459, 17170452, 17170456, 17170454);
    }

    private void getGuestInboxData(boolean z) {
        ...
        if (isNetworkConnected()) {
            ...
            if (this.presenter != null) {
                this.presenter.getGuestInboxData(getString(R.string.please_wait), z);
                return;
            }
        }
    ...
    private void setupViewPager(ViewPager viewPager) {
        ...
        Fragment newInstance = InboxMessagesFragment.newInstance(true);
    ...
    public void onError(APIError aPIError, int i) {
        if (i != ErrorCodes.API_ERROR) {
            if (i == ErrorCodes.INTERNAL_SERVER_ERROR) {
                aPIError = getString(R.string.internal_server_error);
            } else if (i == ErrorCodes.SOCKET_TIME_OUT_ERROR_CODE) {
                aPIError = getString(R.string.socket_time_out_error);
            } else if (i != 511) {
                aPIError = getString(R.string.general_api_error_msg);
            } else {
                aPIError = getString(R.string.network_error);
            }
    ...
    public void readNotification(ReadNotificationEvent readNotificationEvent) {
        if (readNotificationEvent != null) {
            this.presenter.getGuestInboxData(getString(R.string.please_wait), false);
        }
    }
